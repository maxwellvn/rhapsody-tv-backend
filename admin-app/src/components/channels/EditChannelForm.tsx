...
import { UpdateChannelRequest, Channel } from '@/types/api.types';
...
const EditChannelForm = ({ channel, onSuccess, onCancel }: EditChannelFormProps) => {
  ...
  // Auto-update slug when name changes (if slug matches old name)
  const handleNameChange = (value: string) => {
    form.setValue('name', value);
    if (slugValue === generateSlug(nameValue)) {
  ...
  const onSubmit = async (data: ChannelFormValues) => {
    ...
    try {
      ...
      const response = await channelService.updateChannel(channel.id, payload);
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-semibold text-black">Edit Channel</h2>
        {onCancel && (
          <Button
            variant="ghost"
            size="icon"
